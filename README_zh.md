...
> **Helm 目前处于 Beta 测试阶段（v0.1.0）**，欢迎早期用户体验并反馈！
...
> **前置条件：** [Docker](https://docs.docker.com/get-docker/) 和 Docker Compose v2，无需其他依赖。
...
APP_SECRET_KEY=随机字符串密钥
APP_URL=http://你的域名             # 或 http://localhost

EVE_CLIENT_ID=你的EVE应用ID        # 在 developers.eveonline.com 申请
EVE_CLIENT_SECRET=你的EVE应用密钥
EVE_CALLBACK_URL=http://你的域名/auth/eve/callback

JWT_SECRET_KEY=随机字符串JWT密钥
DB_URL=postgresql+asyncpg://helm:你的数据库密码@postgres:5432/helm
```

在 [developers.eveonline.com](https://developers.eveonline.com) 注册应用，回调地址填 `http://你的域名/auth/eve/callback`。
...
首次启动时，Helm 会在日志中打印一条一次性授权链接，用于为首个用户授予管理员权限：

```bash
docker compose -f docker-compose.prod.yml logs backend | grep "HELM SETUP" -A 8
```

先通过 EVE SSO 登录，再在同一浏览器中打开日志里的链接 — 管理员权限立即生效，无需重新登录。

> 详细说明：[Docker 部署指南](https://jerry-scintilla.github.io/Helm-docs/getting-started/docker/)
...
2. 将回调 URI 设置为 `http://localhost:8000/api/v1/auth/eve/callback`
3. 将 `EVE_CLIENT_ID` 和 `EVE_CLIENT_SECRET` 填入 `.env`
